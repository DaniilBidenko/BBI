"use client";

import Link from "next/link";
import { Container } from "./Container";
import { withLocale } from "@/i18n/paths";
import type { Locale } from "@/i18n/config";
import { PrivacyPolicyModalLink } from "./PrivacyPolicyModalLink";

const IconInstagram = ({ className }: { className?: string }) => (
  <svg width="1em" height="1em" viewBox="0 0 24 24" fill="currentColor" aria-hidden className={className}>
    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
  </svg>
);

const IconFacebook = ({ className }: { className?: string }) => (
  <svg width="1em" height="1em" viewBox="0 0 24 24" fill="currentColor" aria-hidden className={className}>
    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
  </svg>
);

const IconWhatsApp = ({ className }: { className?: string }) => (
  <svg width="1em" height="1em" viewBox="0 0 24 24" fill="currentColor" aria-hidden className={className}>
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
  </svg>
);

const IconTelegram = ({ className }: { className?: string }) => (
  <svg width="1em" height="1em" viewBox="0 0 24 24" fill="currentColor" aria-hidden className={className}>
    <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z" />
  </svg>
);

const IconLinkedIn = ({ className }: { className?: string }) => (
  <svg width="1em" height="1em" viewBox="0 0 24 24" fill="currentColor" aria-hidden className={className}>
    <path d="M20.45 20.45h-3.55v-5.57c0-1.33-.03-3.05-1.86-3.05-1.86 0-2.14 1.45-2.14 2.95v5.67H9.35V9h3.41v1.56h.05c.47-.9 1.64-1.85 3.37-1.85 3.61 0 4.27 2.38 4.27 5.47v6.27ZM5.34 7.43a2.06 2.06 0 1 1 0-4.12 2.06 2.06 0 0 1 0 4.12ZM7.11 20.45H3.56V9h3.55v11.45ZM22.23 0H1.77C.8 0 0 .77 0 1.72v20.56C0 23.23.8 24 1.77 24h20.46c.97 0 1.77-.77 1.77-1.72V1.72C24 .77 23.2 0 22.23 0Z" />
  </svg>
);

type FooterLocation = {
  city: string;
  address?: string;
  phone?: string;
  email?: string;
  socials?: { instagram?: string; facebook?: string; whatsapp?: string; telegram?: string; linkedin?: string };
};

export type FooterProps = {
  footer: {
    copyright: string;
    tagline?: string;
    aboutTitle: string;
    aboutLinks: { label: string; href: string }[];
    socialTitle: string;
    locations: FooterLocation[];
    addressLabel: string;
    phoneLabel: string;
    emailLabel: string;
    workingHoursLabel?: string;
    workingHours?: string;
    privacyLabel?: string;
    privacyHref?: string;
  };
  locale: Locale;
  variant?: "default" | "page";
};

function SocialIcon({
  href,
  children,
  label,
  large = false,
}: {
  href: string;
  children: React.ReactNode;
  label: string;
  large?: boolean;
}) {
  return (
    <a
      href={href}
      target="_blank"
      rel="noopener noreferrer"
      className={`flex items-center justify-center rounded-lg bg-[var(--bbi-red)] text-white transition hover:opacity-90 ${large ? "h-12 w-12" : "h-9 w-9"}`}
      aria-label={label}
    >
      {children}
    </a>
  );
}

export function Footer({ footer, locale, variant = "default" }: FooterProps) {
  const privacyUrl = footer.privacyHref ? withLocale(locale, footer.privacyHref) : "";
  const isPage = variant === "page";
  const pageIconClass = isPage ? "h-6 w-6" : "h-4 w-4";

  return (
    <>
      <footer
        className={`site-footer relative z-10 text-white ${isPage ? "mt-0 h-full min-h-full flex flex-col" : "mt-10 md:mt-12"}`}
      >
        <div className="pointer-events-none absolute inset-0 -z-10 bg-[var(--bbi-ambient-bg)]" />
        <div
          className={`w-full bg-[var(--bbi-panel-bg)] pt-12 pb-10 md:pt-14 md:pb-12 ${isPage ? "flex-1 flex flex-col justify-end rounded-none pt-8 pb-6 md:pt-10 md:pb-8" : "rounded-t-3xl border-t border-[color:var(--bbi-panel-border)]"}`}
        >
          <Container className="px-4 lg:px-6">
            <div
              className={`grid gap-7 ${isPage ? "mx-auto max-w-5xl sm:grid-cols-2 lg:grid-cols-2 text-center" : "sm:grid-cols-2 lg:grid-cols-3 lg:gap-10"}`}
            >
              {!isPage && (
                <div className="max-w-xs">
                  <Link
                    href={withLocale(locale, "/")}
                    className="inline-flex items-center"
                    aria-label="BBI — на главную"
                  >
                    <span className="logo-wordmark !h-[1.4rem] !w-[7.8rem]" role="img" aria-hidden />
                  </Link>
                  {footer.tagline && (
                    <p className="mt-3 text-[13px] leading-[1.5] text-white/72">
                      {footer.tagline}
                    </p>
                  )}
                </div>
              )}

              <div>
                <h3 className={`${isPage ? "text-[15px]" : "text-[13px]"} font-semibold text-white`}>
                  {footer.aboutTitle}
                </h3>
                <ul className="mt-3 space-y-2">
                  {footer.aboutLinks.map((item) => (
                    <li key={item.href}>
                      <Link
                        href={withLocale(locale, item.href)}
                        className={
                          isPage
                            ? "text-[16px] font-medium text-white/90 transition hover:text-white"
                            : "text-[13px] text-white/72 transition hover:text-[var(--bbi-red)]"
                        }
                      >
                        {item.label}
                      </Link>
                    </li>
                  ))}
                </ul>
              </div>

              <div>
                <h3 className={`${isPage ? "text-[15px]" : "text-[13px]"} font-semibold text-white`}>
                  {footer.socialTitle}
                </h3>
                <div className={`mt-3 space-y-5 ${isPage ? "flex flex-col items-center" : ""}`}>
                  {footer.locations.map((loc) => (
                    <div key={loc.city} className={isPage ? "text-center" : ""}>
                      <p className={`${isPage ? "text-[15px]" : "text-[13px]"} text-white/72`}>
                        {loc.city}
                      </p>
                      {loc.socials && (
                        <div className={`mt-2 flex gap-2 ${isPage ? "justify-center" : ""}`}>
                          {loc.socials.instagram && (
                            <SocialIcon
                              href={loc.socials.instagram}
                              label="Instagram"
                              large={isPage}
                            >
                              <IconInstagram className={pageIconClass} />
                            </SocialIcon>
                          )}
                          {loc.socials.facebook && (
                            <SocialIcon href={loc.socials.facebook} label="Facebook" large={isPage}>
                              <IconFacebook className={pageIconClass} />
                            </SocialIcon>
                          )}
                          {loc.socials.whatsapp && (
                            <SocialIcon href={loc.socials.whatsapp} label="WhatsApp" large={isPage}>
                              <IconWhatsApp className={pageIconClass} />
                            </SocialIcon>
                          )}
                          {loc.socials.telegram && (
                            <SocialIcon href={loc.socials.telegram} label="Telegram" large={isPage}>
                              <IconTelegram className={pageIconClass} />
                            </SocialIcon>
                          )}
                          {loc.socials.linkedin && (
                            <SocialIcon href={loc.socials.linkedin} label="LinkedIn" large={isPage}>
                              <IconLinkedIn className={pageIconClass} />
                            </SocialIcon>
                          )}
                        </div>
                      )}
                    </div>
                  ))}
                </div>
              </div>
            </div>
            <div
              className={`mt-8 grid gap-7 border-t border-white/10 pt-7 ${isPage ? "mx-auto max-w-5xl text-center sm:grid-cols-2 lg:grid-cols-3" : "sm:grid-cols-2 lg:grid-cols-3"}`}
            >
              {footer.locations.map((loc) => (
                <div key={loc.city} className={`space-y-3 ${isPage ? "text-center" : ""}`}>
                  {loc.address && (
                    <div>
                      <p className={`${isPage ? "text-[12px]" : "text-[11px]"} font-medium uppercase tracking-wider text-white/45`}>
                        {footer.addressLabel} ({loc.city})
                      </p>
                      <p className={`mt-0.5 ${isPage ? "text-[15px]" : "text-[13px]"} font-medium text-white/88`}>
                        {loc.address}
                      </p>
                    </div>
                  )}
                  {loc.phone && (
                    <div>
                      <p className={`${isPage ? "text-[12px]" : "text-[11px]"} font-medium uppercase tracking-wider text-white/45`}>
                        {footer.phoneLabel} ({loc.city})
                      </p>
                      <a
                        href={`tel:${loc.phone.replace(/\s/g, "")}`}
                        className={`mt-0.5 block ${isPage ? "text-[15px]" : "text-[13px]"} font-medium text-white/88 hover:text-[var(--bbi-red)]`}
                      >
                        {loc.phone}
                      </a>
                    </div>
                  )}
                  {loc.email && (
                    <div>
                      <p className={`${isPage ? "text-[12px]" : "text-[11px]"} font-medium uppercase tracking-wider text-white/45`}>
                        {footer.emailLabel} ({loc.city})
                      </p>
                      <a
                        href={`mailto:${loc.email}`}
                        className={`mt-0.5 block ${isPage ? "text-[15px]" : "text-[13px]"} font-medium text-white/88 hover:text-[var(--bbi-red)]`}
                      >
                        {loc.email}
                      </a>
                    </div>
                  )}
                </div>
              ))}
              {footer.workingHoursLabel && footer.workingHours && (
                <div className={`space-y-3 sm:col-span-2 lg:col-span-1 ${isPage ? "text-center" : ""}`}>
                  <p className={`${isPage ? "text-[12px]" : "text-[11px]"} font-medium uppercase tracking-wider text-white/45`}>
                    {footer.workingHoursLabel}
                  </p>
                  <p className={`mt-0.5 ${isPage ? "text-[15px]" : "text-[13px]"} font-medium text-white/88`}>
                    {footer.workingHours}
                  </p>
                </div>
              )}
            </div>
          </Container>
        </div>

        <div className={`mt-auto border-t border-white/10 bg-[var(--bbi-panel-bg)] ${isPage ? "pt-2 pb-0" : "py-4"}`}>
          <Container className="px-4 lg:px-6">
            <div className={`flex flex-col gap-3 sm:flex-row sm:items-center ${isPage ? "items-center justify-center text-center sm:justify-center" : "items-start justify-between"}`}>
              <p className="text-[12px] text-white/58">
                {footer.copyright}
              </p>
              {footer.privacyLabel && footer.privacyHref && (
                <PrivacyPolicyModalLink
                  href={privacyUrl}
                  title={footer.privacyLabel}
                  className="text-[12px] text-[var(--bbi-red)] transition hover:text-[#ff5a5f]"
                >
                  {footer.privacyLabel}
                </PrivacyPolicyModalLink>
              )}
            </div>
          </Container>
        </div>
      </footer>
    </>
  );
}
